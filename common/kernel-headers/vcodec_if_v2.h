...
typedef enum {
	...
	VCODEC_DEC_PARAM_TOKEN_PARTITION,                     //vp8 multi token partition information
	VCODEC_DEC_PARAM_QUALIFY,
...
typedef enum {
    ...
    DISPLAY_CURRENT_INTERLACE_TOP,
    DISPLAY_CURRENT_INTERLACE_BOTTOM,
    DISPLAY_CURRENT_DEINTERLACE,    //top and bottom field de-interlace 
...
typedef enum {
    OUTPUT_BUFFER_EXTRA_NONE,
    OUTPUT_BUFFER_EXTRA_H264_INTERLACE,
    NUM_OF_OUTPUT_BUFFER_EXTRA
} VCODEC_DEC_OUTPUT_BUFFER_EXTRA_T;

//for parsing header when deinterlace
typedef struct {
    unsigned int    u4ExtraBufferType;              // must be OUTPUT_BUFFER_EXTRA_H264_INTERLACE

    unsigned int    u4BufferVaild;
    ...
    void            *reserved_sps[6];
    unsigned int    u4SliceHeaderValid;
    unsigned int    field_pic_flag;

    int             i4ColorFormatSupported; // mapping to VCODEC_COLOR_FORMAT_T enum (1 bit for each color format)
                                            // 1st bit is for VCODEC_COLOR_FORMAT_YUV420, 2nd bit is for VCODEC_COLOR_FORMAT_YV12, ...
                                            // EX: 0b011 means support YUV420 and YV12, not support NV12
    void            *reserved[13];
} VCODEC_DEC_OUTPUT_BUFFER_EXTRA_INTERLACE_T;
...
#define MAX_REF_FREE_YUV_BUFFER_NUM 32
typedef struct {
