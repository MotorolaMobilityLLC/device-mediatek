...
my $SCAT_LK_FILE = "$PRODUCT_OUT/$ArgList{PLATFORM}_Android_lk_only.txt";
...
#add by Longcheer Zhaofei for efuse partition
...
#end add by Longcheer Zhaofei for efuse partition
        ...
        if ($partition_layout_process[$partition_idx]->{Partition_Name} eq "vendor")
        {
            if ($ArgList{MTK_A1_FEATURE} ne "yes")
            {
                splice @partition_layout_process, $partition_idx, 1;
        ...
        if ($partition_layout_process[$partition_idx]->{Partition_Name} eq "odm")
        {
            if ($ArgList{MTK_A1_FEATURE} ne "yes")
            {
                splice @partition_layout_process, $partition_idx, 1;
...
#add by Longcheer Zhaofei for oem partition
		if ($ArgList{LCT_OEM_PARTITION} eq "yes")
		{
...
#end add by Longcheer Zhaofei for oem partition
...
sub GenYAMLScatLKFile()
{
    my $ScatterFileFH = &open_for_rw($SCAT_LK_FILE);
    my %Scatter_Info;
    my $iter;
    for ($iter = 0 ; $iter < @partition_layout_cooked ; $iter++)
    {
...
__TEMPLATE
    ...
    foreach my $part (@partition_layout_cooked)
    {
        if (($part->{Partition_Name} ne "lk") && ($part->{Partition_Name} ne "preloader"))
        {
            print $ScatterFileFH "${FirstDashes}partition_index${colon}SYS$Scatter_Info{$part->{Partition_Name}}{partition_index}\n";
            print $ScatterFileFH "${FirstSpaceSymbol}partition_name${colon}$part->{Partition_Name}\n";
            print $ScatterFileFH "${FirstSpaceSymbol}file_name${colon}$Scatter_Info{$part->{Partition_Name}}{file_name}\n";
...
sub GenYAMLScatFactoryFullFile()
{
    my $ScatterFileFH = &open_for_rw($SCAT_FACTORY_Full_FILE);
    my %Scatter_Info;
    my $iter;
    for ($iter = 0 ; $iter < @partition_layout_cooked ; $iter++)
    {
...
sub GenPGPTFile
{
   ...
   print FH pack("C524288",0x0); #pgpt current max size = 512K --->jade is (34 - 1[PMBR]) LBA = 33*512 = 16896 bytes
   ...
   seek(FH,0x20,0); #Put sgpt partition size here for lk update usage, 6755->16.5KB   D1 -> 512KB
   ...
   seek(FH,0x28,0); #notice 512KB for D1
   for ($piter = 0 ; $piter < @partition_layout_cooked ; $piter++)
   {
       my $part = $partition_layout_cooked[$piter];
